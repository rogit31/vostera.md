The cellars of [[Avendor]] is the name used to reference the secure basement situated underneath the [[Academy Arcanum|academy]]'s campus in Avendor.  

The cellars should include the following sections: 

## Cells
The cells are full of a variety of different criminals that are being experimented on using liselos. 
## Scroll Reserves

## Forbidden books

## Forbidden artefacts
That are mostly items from the [[Titanic Age]].

## Railway station
## Labs 

Includes dry and wet labs, some are research on the old wolrd, some are surgery and medical labs where the cellmates are experimented on

## Temporary barracks

![[cellars.png]]
